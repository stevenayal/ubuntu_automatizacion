...
# Práctica 2 - Automatización Completa (Versión Optimizada)
# Script que ejecuta todos los ejercicios de la Práctica 2 con optimizaciones

echo "=== INICIANDO PRÁCTICA 2 - AUTOMATIZACIÓN COMPLETA (VERSIÓN RÁPIDA) ==="
...
echo "1. Creando archivo plaintext con ls -R /home (optimizado)"
# Usar /home en lugar de / para evitar el problema de tiempo
ls -R /home > plaintext 2>/dev/null || ls -la /home > plaintext 2>/dev/null || echo "Información del sistema" > plaintext
...
if command -v wget &> /dev/null; then
    echo "Intentando descargar archivo de prueba..."
    # Usar un archivo más pequeño para pruebas
    wget -O 200MB.zip "https://speed.hetzner.de/100MB.bin" 2>/dev/null || echo "Descarga no disponible"
    
    if [ ! -f "200MB.zip" ] || [ ! -s "200MB.zip" ]; then
...
echo "Tamaño del archivo: $(ls -lh 200MB.zip | awk '{print $5}')"
echo ""
...
if command -v wget &> /dev/null; then
    ...
    echo "Creando archivo de prueba para verificación..."
...
echo "=== PRÁCTICA 2 COMPLETADA (VERSIÓN RÁPIDA) ===" 