...
# =============================================================================
# EJECUTAR TAREA COMPLETA - PRÁCTICA 4 - ANA
# =============================================================================
...
crear_directorios() {
    ...
    echo "Documento de ejemplo para ana - $(date)" > "$DIR_ORIGEN_DOCS/documento_ejemplo.txt"
    echo "Foto de ejemplo para ana - $(date)" > "$DIR_ORIGEN_FOTOS/foto_ejemplo.jpg"
...
configurar_anacron() {
    ...
    if ! grep -q "respaldo_semanal_ana" /etc/anacrontab 2>/dev/null; then
        echo "7 5 respaldo_semanal_ana $SCRIPT_RESPALDO" | sudo tee -a /etc/anacrontab >/dev/null
...
verificacion_final() {
    ...
    grep "respaldo_semanal_ana" /etc/anacrontab 2>/dev/null || echo "No hay entrada en anacron"
    ...
    if [ -d "$DIR_BACKUP" ]; then
        echo "✓ $DIR_BACKUP"
        ls -la "$DIR_BACKUP" | grep "respaldo_ana" | tail -3 || echo "  No hay respaldos aún"
    else
