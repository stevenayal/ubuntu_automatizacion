...
# =============================================================================
# SCRIPT DE RESPALDO AVANZADO - USUARIO ANA
# =============================================================================
...
# Configuración específica para el usuario ana
USUARIO="ana"
DIRECTORIOS_ORIGEN=(
    "/home/ana/documentos"
    "/home/ana/fotos"
...
NOMBRE_RESPALDO="respaldo_ana_${FECHA_ACTUAL}.tar.gz"
...
# Función para limpiar respaldos antiguos (mantener solo los últimos 7 para ana)
limpiar_respaldos_antiguos() {
    local max_respaldos=7
    local respaldos_actuales=$(ls -t "$DIRECTORIO_DESTINO"/respaldo_ana_*.tar.gz 2>/dev/null | wc -l)
    
    if [ "$respaldos_actuales" -gt "$max_respaldos" ]; then
        show_message "$YELLOW" "Limpiando respaldos antiguos..."
        ls -t "$DIRECTORIO_DESTINO"/respaldo_ana_*.tar.gz 2>/dev/null | tail -n +$((max_respaldos + 1)) | xargs rm -f
...
mostrar_estadisticas() {
    local total_respaldos=$(ls "$DIRECTORIO_DESTINO"/respaldo_ana_*.tar.gz 2>/dev/null | wc -l)
...
main() {
    show_message "$BLUE" "=========================================="
    show_message "$BLUE" "SISTEMA DE RESPALDO AVANZADO - ANA"
    show_message "$BLUE" "=========================================="
