#!/bin/bash
# Completa la preparación del entorno para ana_duarte con sudo
set -e
...
echo \"Este es un archivo de prueba para ana_duarte\"" > /mnt/z/ISO/ubuntu/challenges/challenge_20'
    sudo chmod +x /mnt/z/ISO/ubuntu/challenges/challenge_20
fi
...
echo \"Este es el contenido que se redirige\"" > /mnt/z/ISO/ubuntu/challenges/redireccionar'
    sudo chmod +x /mnt/z/ISO/ubuntu/challenges/redireccionar
fi
...
echo \"Hello World desde ana_duarte!\"" > /mnt/z/ISO/ubuntu/challenges/hello_executable'
    sudo chmod +x /mnt/z/ISO/ubuntu/challenges/hello_executable
fi

# 4. Crear compile_me.c válido
sudo bash -c 'cat > /mnt/z/ISO/ubuntu/challenges/compile_me.c << "EOF"
#include <stdio.h>
int main() {
...
sudo -u ana_duarte bash -c 'cd /mnt/z/ISO/ubuntu/challenges && for i in {1..3}; do echo "archivo deleteme $i" > deleteme_$i.txt; done'

# 8. Asegurar que people.csv tenga datos reales
sudo -u ana_duarte bash -c 'cat > /mnt/z/ISO/ubuntu/challenges/people.csv << "EOF"
...
sudo -u ana_duarte bash -c 'mkdir -p /mnt/z/ISO/ubuntu/challenges/bunch_of_files'
sudo -u ana_duarte bash -c 'echo "You found the needle in the haystack!" > /mnt/z/ISO/ubuntu/challenges/bunch_of_files/needle.txt'
sudo -u ana_duarte bash -c 'echo "archivo de prueba 1" > /mnt/z/ISO/ubuntu/challenges/bunch_of_files/file001.rand'
sudo -u ana_duarte bash -c 'echo "archivo de prueba 2" > /mnt/z/ISO/ubuntu/challenges/bunch_of_files/file002.rand'
sudo -u ana_duarte bash -c 'echo "archivo duplicado" > /mnt/z/ISO/ubuntu/challenges/bunch_of_files/file001.rand.copy'

# 10. Crear archivos de prueba adicionales
sudo -u ana_duarte bash -c 'echo "Hello from greeting1" > /mnt/z/ISO/ubuntu/challenges/greeting1.txt'
sudo -u ana_duarte bash -c 'echo "Hello from greeting2" > /mnt/z/ISO/ubuntu/challenges/greeting2.txt'
sudo -u ana_duarte bash -c 'echo -e "Linea1\nLinea2\nLinea3\nLinea4\nLinea5" > /mnt/z/ISO/ubuntu/challenges/README.txt'
